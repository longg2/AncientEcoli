...
MedianLength <- ecoli %>% group_by(Sample,Length) %>% summarize(Reads = sum(Reads)) %>% summarize(Length = median(rep(Length, Reads))) %>%
	mutate(Sample = NameEdits(Sample))
MedianLength$Pos <- c(3e3,1e4,3e4,1e5)
segmentPlot <- MedianLength %>% left_join(ecoli %>% mutate(Sample = NameEdits(Sample)))
...
FLDfigure <- reduce(list(ecoli),bind_rows) %>% mutate(Sample = NameEdits(Sample)) %>%
	mutate(Organism =factor(Organism, levels = c("Escherichia coli","Homo sapiens","Klebsiella aerogenes")))  %>%
	ggplot(aes(x = Length, y = Reads, colour = Sample)) +
	geom_segment(data = segmentPlot, aes(x = Length, y = Reads, xend = Length, yend = 0), lty = 1) +
	geom_point() + #facet_grid(Organism ~.) + 
	scale_colour_manual(values = colourList) + theme_bw() +
	ylab("Reads") + scale_y_log10(limits = c(1,10^6), breaks = c(1,10,100,10^3,10^4, 10^5,10^6)) + scale_x_continuous(breaks = pretty_breaks(n = 10)) +
	theme(legend.position = "bottom", strip.text.y = element_text(face = "italic")) + annotation_logticks(sides = "l") +
	...
	annotate(geom = "text", x = 100, y = MedianLength$Pos, label = MedianLength$Length, colour = colourList)# + ylab(bquote(log[10]("Reads")))
FLDfigure <- reduce(list(ecoli),bind_rows) %>% mutate(Sample = NameEdits(Sample)) %>%
	mutate(Organism =factor(Organism, levels = c("Escherichia coli","Homo sapiens","Klebsiella aerogenes")))  %>%
	ggplot(aes(x = Length, y = Reads, colour = Sample)) +
	geom_point() + #facet_grid(Organism ~.) + 
...
figure <- reduce(list(human, ecoli), bind_rows) %>%
	...
	ggplot(aes(x = Mismatches, y = Prop, color = Sample, label = Count)) + geom_point() + geom_line() +
...
ggsave(figure, file = "~/Documents/University/EcoliPaperV2/Figures/Mismatches.pdf", width = 12, height = 9)
...
annotationDF <- ecoli %>% group_by(Sample) %>% filter(Pos == -25)
annotationDF$Pos <- c(0.1,0.125,0.15,0.175)
annotationDF <- annotationDF %>% filter(Sample != "Digest 1")
...
mapDamagefigure <- reduce(list(ecoli),bind_rows) %>% filter(Sample != "Digest 1") %>%
	...
	annotate(geom = "text", x = -10, y = annotationDF$Pos, label = signif(annotationDF$DamageFrac,3), colour = colourList[-1])

	#annotate(geom = "text", x = 10, y = , label = c("5`", "3`"), fontface = "bold")

# Now to make the Plasmid MapDamage Plot
plasmid <- mapDamageParsing("PlasmidMapDamage")
annotationDF <- plasmid %>% group_by(Sample) %>% filter(Pos == -25)
annotationDF$Pos <- c(0.1,0.125)

mapDamagePlasmid <- plasmid %>% ggplot(aes(x = Pos, y = DamageFrac, col = Sample)) + geom_line() + theme_bw()+
	scale_colour_manual(values = colour) +# facet_grid(Organism ~.) +
	xlab("Distance from Center") + ylab("Fraction Damaged") +
	coord_cartesian(ylim = c(0,0.25)) + geom_vline(xintercept = 0, lty = 2, size = 1) +
	theme(strip.text.y = element_text(face = "italic"), legend.position = "bottom") +
	annotate(geom = "text", x = -10, y = 0.15, label = "Maximum CT Deamination") +
	annotate(geom = "text", x = -10, y = annotationDF$Pos, label = round(annotationDF$DamageFrac,3), colour = colour[1:2])

ggsave(mapDamagePlasmid, file = "~/Documents/University/EcoliPaperV2/Figures/PlasmidMap.pdf", width = 9, height = 6)
...
ggsave(file = "~/Figure1V4.pdf", width = 12, height = 8)
